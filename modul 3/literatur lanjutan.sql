...
-- praktek 10
SELECT datedift(now(), tgl_transaksi) FROM tr_penjualan; 
SELECT datedift(now(), tgl_transaksi,now()) FROM tr_penjualan; 
...
FROM tr_penjualan
WHERE nama_produk = 'Flasdisk DQlab 32 gb';
...
-- ambil semua produk yang berawalan huruf 'f' dan qty . 2
